...
**Formal Cursor ↔ [Proxmox VE](https://www.proxmox.com/) MCP integration** — 165 tools covering QEMU VMs (incl. guest-agent network/file push), LXC (incl. day-2 prepare/push/nginx + crun Docker fallback + capabilities), unified guest power, storage admin, cluster/tasks, snapshots, backups (incl. scheduled jobs), migration, HA, firewall (incl. IPSet CIDRs), access control, replication, SDN (read), ACME (read), pools, and console tickets. v1.4.0 ships Phase F.1 VM parity + create `wait=` + optional inventory probes.
...
| **QEMU** | lifecycle + config (ISO/cloud-init/net on create/update; optional `wait=true`) + `get_vm_network` / `push_to_vm` / `pull_from_vm` (guest agent) + `get_vm_status`, `get_vm_rrd_data`, console tickets |
| **LXC** | lifecycle + config + suspend/resume (CRIU warn) + `get_lxc_status` / `get_lxc_network` / `get_lxc_rrd_data` + VNC/SPICE/termproxy; `ssh_public_keys` / `docker_ready` / `nameserver` / `wait` on create; `set_lxc_password` / `set_lxc_ssh_keys` / `execute_lxc_command` / `prepare_lxc_for_docker` (`docker_mode=auto|keyctl|crun`) / `configure_lxc_dns` / `pct_set_lxc` / `push_to_lxc` / `pull_from_lxc` / `deploy_static_nginx` via opt-in **host** SSH + `pct` ([setup](SETUP.md#ssh-for-lxc-exec-opt-in)); optional `get_containers(probes=true)` |
...
- [x] Formal multi-domain Proxmox API coverage (165 tools)
