...
| **LXC** | lifecycle + config + suspend/resume (CRIU warn) + `get_lxc_status` / `get_lxc_network` / `get_lxc_rrd_data` + VNC/SPICE/termproxy; `ssh_public_keys` / `docker_ready` / `wait` on create; `set_lxc_password` / `set_lxc_ssh_keys` / `execute_lxc_command` / `prepare_lxc_for_docker` / `push_to_lxc` / `pull_from_lxc` / `deploy_static_nginx` via opt-in **host** SSH + `pct` ([setup](SETUP.md#ssh-for-lxc-exec-opt-in)); optional `get_containers(probes=true)` |
...
- [x] Formal multi-domain Proxmox API coverage (163 tools)
