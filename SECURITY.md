...
3. Optional **`auth_write`** (D31) for elevated mutations — ACL both token identities; see SETUP.md.
4. Prefer `verify_ssl: true` with a trusted CA; disabling SSL verification is for labs only.
5. Keep `token_value` out of git. Prefer env interpolation in config (see SETUP.md).
6. Tools such as `execute_vm_command` / `execute_lxc_command` can run arbitrary guest commands — scope the API token and (for LXC) the SSH key accordingly. LXC exec is opt-in via config `ssh` + `pct exec` on the host; keep keys dedicated and restrictable.
7. Optional `PROXMOX_MCP_EXEC_ALLOWLIST` regex can block guest commands that do not match.
8. Destructive tools (`delete_*`, restore-overwrite, ACL/firewall changes) are irreversible; review agent plans before approving.
9. Install the PyPI package **`cursor-proxmox-mcp`** — not the unrelated `proxmox-mcp-server` package on PyPI.
